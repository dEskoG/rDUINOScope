...
// Now I'll use the global Variables OBJECT_RA_H and OBJECT_RA_M  To calculate the Hour angle of the selected object.
    ...
    // This will make the telescope to doa Meridian Flip... depending on the current HA and predefined parameter: MIN_TO_MERIDIAN_FLIP
...
int delta_RA_timer = millis() - Slew_RA_timer;
    ...
    if (IS_OBJECT_RA_FOUND == true && IS_OBJECT_DEC_FOUND == true){
      ...
      Serial.print("ALT (deg) = ");
      Serial.println((asin(ALT)*180)/3.1416);
      IS_OBJ_FOUND = true;
...
void setmStepsMode(char* P, int mod){    // P means the axis: RA or DEC; mod means MicroSteppping mode: x32, x16, x8....
...
void Current_RA_DEC(){
      ...
      sprintf(curr_DEC_lz, "%c%02d%c%02d:%02d", sDEC_tel, int(abs(curr_DEC_D)), 223, int(curr_DEC_M), int(curr_DEC_S));
